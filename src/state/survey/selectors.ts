...
const selectCurrentSurvey = (state: any) => getSurveyState(state).currentSurvey;

const selectCurrentSurveyId = (state: any) => selectCurrentSurvey(state)?.id;

const selectCurrentSurveySrsIndex = (state: any) => {
  const survey = selectCurrentSurvey(state);
  return Surveys.getSRSIndex(survey);
};

const selectCurrentSurveyRootDef = (state: any) => {
  const survey = selectCurrentSurvey(state);
  return Surveys.getNodeDefRoot({ survey });
};

const selectIsNodeDefEnumerator = (nodeDef: any) => (state: any) => {
  const survey = selectCurrentSurvey(state);
...
const selectIsNodeDefRootKey = (nodeDef: any) => (state: any) => {
  ...
  return keyDefs.some((keyDef: any) => keyDef === nodeDef);
};
...
const selectCurrentSurveyCycle = (state: any) => getSurveyState(state).currentSurveyCycle;
...
export const SurveySelectors = {
  ...
  useIsNodeDefEnumerator: (nodeDef: any) => useSelector(selectIsNodeDefEnumerator(nodeDef)),
  useIsNodeDefRootKey: (nodeDef: any) => useSelector(selectIsNodeDefRootKey(nodeDef)),
