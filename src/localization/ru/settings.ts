export default {
  ...
  autoSyncOpenRecordIntervalMinutes: {
    label: "Интервал автосинхронизации во время редактирования: {{value}} мин",
    description:
      "Сколько времени запись, открытая в данный момент в редакторе, должна оставаться без изменений, прежде чем автосинхронизация загрузит её",
  },
  autoSyncSlowCheckIntervalMinutes: {
    label: "Интервал автосинхронизации (медленная проверка): {{value}} мин",
    description:
      "Когда все записи уже синхронизированы, как часто автосинхронизация повторно проверяет сервер на наличие изменений (например, с другого устройства). Проверки выполняются чаще, если появляется что-то новое для отправки",
  },
  dataUploadChunkSizeKB: {
    label: "Размер фрагмента при загрузке данных",
    description:
      "Размер каждой части, на которые разбивается запись при загрузке на сервер. Меньшие значения более надёжны при медленном или нестабильном соединении, но требуют больше запросов для передачи того же объёма данных",
  },
  ...
  showRecordCompletion: {
    ...
    description:
      "Отображает индикатор выполнения с процентом выполненных требований текущей записи",
  },
  showStatusBar: {
