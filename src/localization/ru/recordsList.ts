export default {
  cloneRecords: {
    title: "Клонировать",
    confirm: {
      message:
        "Клонировать выбранные {{recordsCount}} записи в цикл {{cycle}}?",
    },
    onlyRecordsImportedInDeviceOrModifiedLocallyCanBeCloned:
      "Только записи, загруженные на устройство или измененные локально, могут быть клонированы в следующий цикл",
    completeSuccessfully: "Записи успешно клонированы в цикл {{cycle}}!",
  },
  confirmFetchRecordFromServer: "Получить запись с сервера?",
  continueEditing: {
    title: "Продолжить редактирование",
    confirm: {
      message: "Продолжить редактирование с того места, где вы остановились?",
    },
  },
  dateModifiedRemotely: "Дата изменения на удаленном сервере",
  deleteRecordsConfirm: {
    title: "Удалить записи",
    message: "Удалить выбранные записи?",
  },
  duplicateKey: {
    title: "Дубликат ключа",
    message: "Другая запись с таким же ключом ({{keyValues}}) уже существует.",
  },
  exportRecords: {
    title: "Экспорт",
  },
  fetchRecord: "Получить запись",
  fetchRecords: {
    title: "$t(common:fetch)",
    error: {
      surveyNotVisibleInMobile:
        "Невозможно получить записи: этот опрос не должен быть виден в Arena Mobile",
      recordsDownloadNotAllowed:
        "Получение записей с сервера Arena не разрешено",
    },
  },
  importRecordsFromFile: {
    title: "Загрузка",
    confirmMessage: "Загрузить записи из выбранного файла\n{{fileName}}?",
    invalidFileType: "Неверный тип файла (ожидается .zip)",
    overwriteExistingRecords: "Перезаписать существующие записи",
    selectFile: "Выберите файл",
  },
  importCompleteSuccessfully:
    "Загрузка записей успешно завершена!\n- Обработано {{processedRecords}} записей\n- Добавлено {{insertedRecords}} записей\n- Обновлено {{updatedRecords}} записей",
  importFailed: "Ошибка загрузки записей: {{details}}",
  loadStatus: {
    title: "Загружено",
    C: "Полностью",
    P: "Частично (без файлов)",
    S: "Только сводка",
  },
  origin: {
    title: "Источник",
    L: "Локальный",
    R: "Удаленный",
  },
  owner: "Владелец",
  recordHasErrorsOrWarningsTooltip: "Запись содержит {{count}} {{itemsTypeText}}",
  recordType: {
    all: "Все записи",
    local: "Записи на устройстве",
  },
  revalidateRecords: {
    title: "Повторная проверка",
    allRecordsTitle: "Повторно проверить все записи",
    confirm: {
      message:
        "Пересчитать проверку для выбранных {{recordsCount}} записей? Используйте это, если запись показывает ошибки или предупреждения, которые вы не можете найти при её редактировании.",
    },
    completeSuccessfully: "Записи успешно перепроверены!",
  },
  sendData: {
    error: {
      generic: "Невозможно отправить данные на сервер: {{details}}",
      surveyNotVisibleInMobile:
        "Текущий опрос не должен быть виден в Arena Mobile",
      recordsUploadNotAllowed: "Загрузка записей на сервер не разрешена",
      recordsWithErrorsUploadNotAllowed:
        "Загрузка записей с ошибками валидации на сервер не разрешена",
    },
  },
};
