...
最大数に達しました`,
  ...
  recordEditor: "記録エディタ",
  recordCompletion: {
    description: "レコードの要件のうち{{percent}}%が満たされています",
...
サーバーから記録一覧を取得して再試行しますか？`,
  ...
  coordinate: {
    ...
    keepXAndY: "XとYを保持",
    locationHeadingNotAvailable: "GPS方位が利用できません！",
    magnetometerNotAvailable: "方位センサーが利用できません！",
    navigateToTarget: "目標に向かってナビゲート",
    srs: "$t(common:srs)",
    targetLocation: "目標位置",
    useCurrentLocation: "現在地を使用",
    usingInternalFallback: "内部GPSを使用中（{{label}} は利用できません）",
    usingSource: "{{label}} を使用中",
    x: "X",
    y: "Y",
